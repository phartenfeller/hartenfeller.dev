---
  ...
  date: 2024-05-06
  description: My upgrade journey from dockerized Oracle 23c to 23ai
...
[Last week a new Oracle release dropped](https://blogs.oracle.com/database/post/oracle-23ai-now-generally-available) . Instead of a completely new release it is an update of the 23c build with a few highly anticipated changes like vector support and updates to JSON duality views. Instead of the "c" prefix reffering to cloud they are now marketing it as 23ai release. Currently it is avalable in the cloud and you can run "FREE" release on your own hardware.
...
As we will take some time to backup everything we need to stop users from being able to change data. As I only have an ORDS that connects users to the data I simply stopped it. You might want to disconnect other users or disable writes.

```bash
docker stop oracle-23c-free-ords
```

## Export data from old 23c
...
## Start new 23ai database container
...
You can follow the DB init process with `docker logs oracle-23ai-free --follow`. This can take some time depending on your hardware.
...
To make our life easier we can go to the folder where we exported all the APEX stuff and call `ls -A1` . Copy the the file names into a new `install-all.sql` file and prefiex them with `@`. Move the `w` prefixed workspace files to the top.
...
Now we can connect to the FREEPDB1 and install all workspaces and apps with a single script:
...
Now the data and APEX stuff should be back as it was. We can now change our ORDS config to point to our new database.
