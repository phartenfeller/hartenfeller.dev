...
As stated above everyone can execute that small snippet by themselves. So **hiding the button is no security measure**. What we can do to only allow privileged users to trigger processes is to put server-side condition or authorization schemes onto them in APEX:
