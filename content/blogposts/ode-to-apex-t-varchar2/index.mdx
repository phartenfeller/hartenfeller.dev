...
With normal arrays, you can already do great stuff but `apex_t_varchar2` gets its superpowers because of the [apex_string](https://docs.oracle.com/en/database/oracle/apex/22.1/aeapi/APEX_STRING.html#GUID-CAFD987C-7382-4F0F-8CB9-1D3BD05F054A) package which is a collection of functions to work with them.
...
You can only call the API with one customer code at a time, but the user wants to fetch multiple ones most of the time. We can improve this by allowing to pass multiple customer codes as a comma-separated string (CUST1,CUST2,CUST3):
...
If you want to filter the values based on a number column you can use the function `apex_string.split_numbers` which returns `apex_t_varchar2`'s sibling `apex_t_number`:
