...
Cronicle also has a REST-API that offers basic tasks like start a job, check if it has finished, and get the log. You can pretty easily start a job from GitHub actions with the following script:
...
Because it just starts the Job we don't get any info on what happens. In the GitHub Actions log you just see wether the API call worked and the same as log. There is no info on what goes on on the server.

This was a problem for another use case. I want to run a database install scripts in a local database when a Pull Request was created at GitHub. Ideally the Pull-Request should then show if this Action-Run was successful or not and you should be able to jump to the log output to find out what is broken within GitHub.

## The Plugin

I came to the conclusion that the easiest solution would be to write a GitHub Actions-Plugin specifically for Cronicle with a simple API. Here is an example:
