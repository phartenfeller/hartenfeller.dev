...
- Add following target: `#action$edit-salary?id=&ID.&sal=&SALARY.`
- Add ID to button in Link Attributes: `id="SAL-BTN-&ID."`

Note that we refrence the two columns **ID** and **SALARY** in the target URL and the ID attribute.
...
The ID column is also used to add a HTML ID to the button we can later use to reference it and position the popup.

The link target uses the APEX actions API. <Link to="/blog/apex-actions-links-22-1">You can find more about it here.</Link>

**Create the popup**

We create the popup region like we did before.
Afterwards we cann add two page items to the region.
First the salary input field and second a hidden one that will store the ID of the current edited row.

We also add a button to invoke the save process.

We again need to add a `data-parent-element` attribute to the popup region. This seems odd but we need to reference anything static on the page. For example any button. We need APEX to run the initialisation binding code so we can modify the result later:
...
We still need to actually open the popup.
...
Afterwards we can run "Close Dialog" and if we want refresh the cards region to reflect the changes.
...
I just found out that the popup region stores the jQuery magic that binds popup to its anchor in the `apexPopup` data attribute.
After changing some properties of this object we can make the popup appear at the a different position.

Et voilà! We are done and the popup will no position dynamically to the clicked button.
