...
After setting the ID item we can refresh the detail report in the modal via a `Refresh` action (make sure `page items to submit` is set to the changed ID item). Afterward, we can open the modal with the `Open Region` action.
...
In the link tag, we can now get rid of our data attribute as well as the class. Instead, we change the link to the following:
...
As I still develop mostly in older versions of APEX I thought there could be a way to still user actions in the same manner there. As I thought only the link `href` syntax for actions was new I thought I could achieve the same effect with some JavaScript from the `href`. The actions API has an invoke method that can be used to trigger manually actions:
...
So in this case I still need to add data attributes with IDs to the links and manually listen for the events as I need the event source to grab that ID.
