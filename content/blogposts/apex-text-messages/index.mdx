...
Text messages are a feature that I utilise not enough in APEX. They are easy to understand and implement, but I have to remind myself that they exist, so I am writing this blog post.

They are basically translatable text fragments that you can reference almost anywhere. My rule of thumb is if you either need a specific text in multiple languages or multiple times you should create a text message if you don't want your own logic with a separate table.
...
When you go to shared components you can navigate to text messages on the bottom right section. You see and edit existing ones or create new ones.
...
When you created a text message you can use it in your app in different ways.
...
You can use text messages from PL/SQL. The `APEX_LANG` package has the function `message` for this purpose:

```sql
apex_lang.message(p_name => 'GREETING')
```

You can additional parameters that will be replaced in the text message. The text message can include placeholders `%0` to `%9` which will be replaced by the corresponding parameter:
...
It would be inefficient when all text messages would be loaded on every page load even when you don't use them. Thats why we need to explicitly load them before usage. You can do this with the `apex.lang.loadMessages` function. It takes an array of message IDs and returns a promise. When the promise resolves you can use the messages.

Strangely, the API to access the messaages is different. You have to use the `apex.lang.getMessage` function to get a message and additionally `apex.lang.formatMessage` to format ones with parameters. You can't pass the parameters as an object with named parameters, instead you have to pass them as separate arguments one after another.
...
As `loadMessages` calls the database and this is asynchronous, the function returns a [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise). From the context of the browser the database is a completely different system. We send a message to it and hope that after some time it fulfills our request and returns something (the messages).
...
To be able to use `await` we have to put our code into a function and label it as asynchronous by writing `async` before it. We could have wrapped the code into a named function and immediatly called it like `async function run() { ... } run();`. I like to use [IIFEs](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) (Immediately Invoked Function Expression) where you don't give a name to the function and call it directly by putting these strange sets of "()" in there.

</InfoBox>

## Auto use of translated text messages

If you have a multi language app you can translate your text messages. If you start creating a new translation your existing text messages will be copied and you can translate the text. If you add a new text message you have to manually create another one for the other language.

If you use the substitution strings in your app, APEX will automatically use the text message of the users languages if it exists. This is really convinient.

I haven't worked a lot with translated apps but [FlowsForAPEX](https://flowsforapex.org/) builds on it. You can check out their builder app to see it in action. Here is one example for the message "APP_VIEW"
...
## Difference to shortcuts

In the shared components under "Other Components" there are also shortcuts. The help text says: "Use Shortcuts to write frequently used code once and then reference it in many places within your application. Shortcuts also allow for dynamic generation of code in places that typically only support static text."

<BlogImg filename="apex_messages_shortcuts.png" alt="" />

They work in a similar fashion as they also allow to store text and you reference them with some kind of substitution syntax. You need to wrape the name in paranthesis like `"MY_SHORTCUT"`. They do not allow translations or parameters and don't have a nice API like the text messages have.

They allow for dyanmic content as you can call a PL/SQL function that returns a string. There is even a type form images but I have no clue how and when to use it:
...
My take is that they are more focused on HTML content rather than basic text content. I have not yet found a use case for them in practice, if you have please share it in the comments.
