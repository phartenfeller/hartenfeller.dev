...
```ts head="src/lib/sqlite/types.ts"
...
```ts head="src/lib/sqlite/worker.ts"
...
```ts head="src/lib/sqlite/loadWorker.ts"
import { WorkerMessageTypes, type WorkerMessage } from './types';
...
```ts head="src/lib/sqlite/initDb.ts"
...
```html head="src/routes/+layout.svelte"
<script>
