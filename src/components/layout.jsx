...
const Layout = ({ children, header = false, toc, stickyHeader = false }) => (
  <div className="ite flex min-h-screen flex-col">
          ...
          {toc && toc.items ? (
            <Menu as="div" className="flex">
              {({ open }) => (
                <>
                  <Menu.Button
                    className="mr-3 flex items-center rounded px-4 py-2 align-middle text-red-100 text-opacity-60 focus:outline-none focus:ring focus:ring-red-400"
                  ...
                  >
                    <Menu.Items
                      className={`absolute right-0 z-50 ml-2 mt-8 origin-top-right overflow-hidden
    ...
    <footer className="md:text-md grid grid-cols-2 bg-title-brown-light px-8 py-4 text-sm text-white shadow-lg md:grid-cols-3 md:font-semibold lg:grid-cols-5">
